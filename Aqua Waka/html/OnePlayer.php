...
<head>
	...
	<script type="text/javascript">
		...
		var objects = [];
		var keys = {};
		var cube, cube2, cubeSmash, cube3;
		var isWorldReady = [false, false];
		var texturewater;
		var objs = [];
		var cubesCol = [], cubesMov = [];
		var i = 0;
		var movpos = 0, moval = 0.001;
		var cambioNiveles = 0;
		var contador = false;
		var contadores = [];
		//Colisiones var
		var collidableMeshList = [];
		// var particleTexture, particleGroup, particleAttributes;
		...
		function render(){
			...
			if (isWorldReady[0]) {
				...
				// cube3.translateY (movpos * deltaTime)
				// Mov_Y(cube, mov[0]);
		...
		function setupScene() {		
			...
			//Acomodar posiciones de cubos en modelos

			// Cubos de mov nivel 1
			cubesMov[2] = createCubesMov(geometry, material, cubesMov[2], 0, 0.5, 38, 5, 5, 5);
		...
		function Colisiona(CuboMazo){
				...
				for(var vertexIndex = 0; vertexIndex < CuboMazo.geometry.vertices.length; vertexIndex++){
					...
					// && collisionResults[0].object.name == "cubo5"
		...
		function createParticle(PparticleT, PparticleG, PparticleA, Ptotal, Pradius){
			for( var i = 0; i < Ptotal; i++ ) 
			{
				var spriteMaterial = new THREE.SpriteMaterial( { map: PparticleT, useScreenCoordinates: false, color: 0xffffff } );
				...
				// for a solid sphere:
				// sprite.position.setLength( Pradius * Math.random() );
				...
				PparticleG.add(sprite);
				// add variable qualities to arrays, if they need to be accessed later
				PparticleA.startPosition.push( sprite.position.clone() );
				PparticleA.randomness.push( Math.random() );
			}
			PparticleG.position.y = 50;
				// particleGroup.position.x = 50;
			scene.add(PparticleG);
		}
		...
		function Mov_Y(cubesy, valor, valor2){
			if (contador == false){
				if(cubesy.position.y < valor){
					cubesy.position.y +=0.1 ;
				}
			}else{
				contador = true;
			}
                    ...
                    <div>
                        <form id="formusername" class="mb-2">
                            <div class="form-group mb-2">
                                ...
                                <input type="image" class="img-fluid" id="btnUp" src="../sources/subir.png" alt="subir" data-bs-dismiss="modal" onclick="writeData(); alert('Datos subidos con exito');">
